...
public:
    ...
    Large(int64_t value) { *this = Large(std::to_string(value)); }

    Large(const Large &other) : digits_(other.digits_), sign_(other.sign_) {}

    Large(Large &&other) noexcept: digits_(std::exchange(other.digits_, std::vector<uint64_t>())), sign_(std::exchange(other.sign_, Sign::Plus)) {}
